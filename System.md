...
> 本ドキュメントは「システム構成(一部実装途上)」を示すものです。
...
- **見守りから介護事業者システムへの連携**: お年寄りとの会話状況をもとに、不安な事項（体調不良の訴え、いつもと違う様子、SOS など）を検知した場合は、介護事業者システム（`Medical/`）へ連携し、専門的な記録化や医療機関（看護師）への通知につなげることを目指します。
- **「ジョーズくん」のぬいぐるみ化**: 最終的には、マスコット「ジョーズくん」を **ぬいぐるみ型のデバイス** として各家庭に置き、画面越しではなく生活空間の中で自然に話しかけ・見守りができる形を目指します。ぬいぐるみに音声入出力（マイク / スピーカー）を内蔵し、本システムの対話・見守り機能を担わせる構想です。
...
- **最終形（想定）**
  ...
  - 会話状況から **不安な兆候を検知した場合、介護事業者システム（`Medical/`）へ連携** し、記録化・医療機関への通知へつなげる
  - **ハードウェア展開**: 最終的には「ジョーズくん」を **ぬいぐるみ型デバイス** として家庭に設置し、ぬいぐるみに内蔵したマイク / スピーカーで見守り対話を行う
...
介護スタッフやお年寄りの口頭メモやテキストを、専門的な介護記録に変換し、医療機関（看護師）との連携を支援します。
...
> 注記: 本構成図は開発途中時点での想定です。特にデモアプリ（`app.html`）の音声出力の ElevenLabs 化、家族向けメール通知、会話状況からの介護事業者システム連携、および「ジョーズくん」のぬいぐるみ型デバイス化は、今後実装・展開を予定している要素です。
